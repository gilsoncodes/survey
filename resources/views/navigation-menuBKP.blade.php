<nav x-data="{ open: false }" class="fixed w-full bg-white border-b border-gray-100">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            ...
            <div class="flex">
                <!-- Navigation Links -->
                <div class="hidden sm:-my-px sm:ml-10 sm:flex"> <!-- Home -->
                ...
                <div class="hidden sm:-my-px sm:ml-10 sm:flex"> <!-- Services -->
                    <x-jet-nav-link href="{{ route('services') }}" :active="request()->routeIs('services')">
                        {{ __('Services') }}
                    </x-jet-nav-link>
                </div>
                <div class="hidden sm:-my-px sm:ml-10 sm:flex"> <!-- Contact -->
                    <x-jet-nav-link href="{{ route('contact') }}" :active="request()->routeIs('contact')">
                        {{ __('Contact') }}
                    </x-jet-nav-link>
                </div>
                <div class="hidden sm:-my-px sm:ml-10 sm:flex"> <!-- About -->
            ...
            <div class="flex justify-between h-16">
                ...
                @else
                <div class="hidden sm:-my-px sm:ml-10 sm:flex"> <!-- Log in -->
                    <x-jet-nav-link href="{{ route('login') }}" :active="request()->routeIs('login')">
                        {{ __('Log in') }}
                    </x-jet-nav-link>
                </div>
                @endauth
                <div class="hidden sm:-my-px sm:ml-10 sm:flex"> <!-- Make an Apppointment -->
                    <a class="inline-flex items-center h-10 px-4 my-auto text-sm text-white transition-colors duration-150 bg-yellow-500 rounded-xl focus:shadow-outline hover:bg-yellow-600" href="{{ URL::route('contact') }}#appointment">
                        {{ __('Make an Apppointment') }}
                    </a>
                </div>
            </div>
            <!-- Hamburger -->
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="flex justify-center"> <!-- Make an Apppointment -->
            <a class="inline-flex items-center h-10 px-4 my-auto text-sm text-white transition-colors duration-150 bg-yellow-500 rounded-xl focus:shadow-outline hover:bg-yellow-600" href="{{ URL::route('contact') }}#appointment">
                {{ __('Make an Apppointment') }}
            </a>
        </div>
